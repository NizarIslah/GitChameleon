...
def main():
    ...
    parser.add_argument("env_dir", help="Path to the dir where the environnement have been created")
    parser.add_argument("test_dir", help="Path to the dir where the test files are stored")
    ...
    for idx, record in tqdm(enumerate(data), total=len(data), desc="Processing JSON lines"):
        ...
        test_folder_path = os.path.join(args.test_dir, f"test_early_sample_{example_id}/")

        # Get the first file starting with "test" in the test folder
        test_files = [f for f in os.listdir(test_folder_path) if f.startswith("test") and f.endswith('.py')]
        assert len(test_files) == 1, f"Expected exactly one test file in {test_folder_path}, but found {len(test_files)}"
