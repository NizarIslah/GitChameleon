...
class TestSample163(unittest.TestCase):
    ...
    def test_data_route(self):
        """Test the /data route with various numpy arrays (duplicates removed, NaNs removed)"""
    ...
    def test_data_with_nan(self):
        """Test the /data route with arrays containing NaN values. We expect duplicates and all NaN values removed."""
        test_arr = np.array([1.0, 2.0, np.nan, 3.0, np.nan])
        with self.app.test_request_context():
            response = data(test_arr)
            response_data = json.loads(response.get_data(as_text=True))
            # We expect [1.0, 2.0, 3.0] with neither duplicates nor NaN.
    ...
    def test_custom_json_encoder(self):
        """Test the custom JSON encoder directly with duplicates removed and NaNs removed."""
        ...
        test_arr = np.array([1.0, 2.0, np.nan, 3.0, np.nan])
        encoded = encoder.default(test_arr)
        # We expect duplicates and all NaNs to be excluded
