...
class TestImaging(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_result[0, 0, 0] = (50 * 100) // 127  # < 128
        self.expected_result[0, 0, 1] = 255 - ((255 - 150) * (255 - 200)) // 127  # >= 128
        self.expected_result[0, 0, 2] = (100 * 50) // 127  # < 128
    ...
    def test_imaging_with_same_size_images(self):
        """Test that the imaging function works with same-sized images (now returning an ndarray)."""
        result = imaging(self.img1_small, self.img2_small)
        self.assertIsInstance(result, np.ndarray)
        # Check the height/width portion of the shape matches our 2x2 input
        self.assertEqual(result.shape[:2], (self.img1_small.size[1], self.img1_small.size[0]))
    ...
    def test_hardlight_calculation(self):
        """Test that the hardlight calculation produces the expected ndarray results."""
        result = imaging(self.img_test1, self.img_test2)
        # Convert result to numpy array for comparison (should already be array, but this is safe)
    ...
    def test_imaging_preserves_image_dimensions(self):
        """Test that the output array has the same height and width as the input."""
        result = imaging(self.img_test1, self.img_test2)
        self.assertEqual(result.shape[:2], (self.img_test1.size[1], self.img_test1.size[0]))
