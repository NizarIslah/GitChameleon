...
# Ensure we can import Gradio and sample_36. If dependencies are missing or incompatible,
# skip all tests rather than failing during import.
try:
    # Add the parent directory to sys.path to locate sample_36
    ...
    # Print Gradio version just to confirm it's found
    gr_version = gr.__version__
    print(f"Using gradio version: {gr_version}")

except ImportError as e:
    # If we cannot import gradio/sample_36 because of a version mismatch or missing dependency,
    # we skip the entire test file.
    raise unittest.SkipTest(f"Skipping tests because of ImportError: {e}")
