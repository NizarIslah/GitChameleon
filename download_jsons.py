...
def extract_json_from_wandb(filter_key, model_name, seed, temp, n_generate, cot=False):
    ...
    filtered_runs = api.runs(path=f"{entity}/{project}",
    ...
    filtered_runs = [
        run for run in filtered_runs if parse_created_at(run.created_at) > filter_date_time
    ]
    assert len(filtered_runs) > 0, "No runs found. Please check the filters."
    #sort by name
    filtered_runs = sorted(filtered_runs, key=lambda x: x.name)
    print(f"Total & Unique runs satisfying the criteria: {len(filtered_runs)}, {len(set([run.name for run in filtered_runs]))}")
    ...
    for run in filtered_runs:
        ...
        for file in run.files():
            if 'outputs' and 'jsonl' in file.name and ("evals" in file.name or filter_key != "evaluate"):
                # print(file.name)
        ...
        for model in tqdm(model_names):
            for cfg in [(seed, 0.0, 1)]: #, (seed, 0.3, 25), (seed, 0.8, 100)]:  # seed, temp, n_sample (seed, 0.3, 25), 
                try:
                    ...
                    try:
                        json_file = extract_json_from_wandb(filter_key, model_alt, cfg[0], cfg[1], cfg[2], cot=cot)
                ...
                if cot:
                    new_name = json_file.name.replace(".jsonl", f"_{cfg[0]}_{cfg[1]}_{cfg[2]}_cot.jsonl")
                else:
                    new_name = json_file.name.replace(".jsonl", f"_{cfg[0]}_{cfg[1]}_{cfg[2]}.jsonl")
                if filter_key == "regenerate":
