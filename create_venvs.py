...
def install_packages(env_path, library, version, additional_dependencies):
    ...
    if str(additional_dependencies) in ('nan', '', '-', 'io'):
        pip_install_cmd = [python_executable, "-m", "pip", "install", f"{library}=={version}", "--quiet"]
    else:
        pip_install_cmd = [python_executable, "-m", "pip", "install", f"{library}=={version}", "--quiet"] + additional_dependencies.split()
    ...
    if result.returncode != 0:
        print(f"Failed to install packages in {env_path}: {result.stderr}, tearing down the environment...")
...
def generate_env_id(row):
    ...
    return hashlib.sha256(unique_str.encode()).hexdigest()[:8]  # Shorten the hash for readability
...
def main():
    ...
    for row_idx, row in df.drop_duplicates(subset=['env_id']).iterrows():
        ...
        returncode = install_packages(env_path, row['library'], row['version'], row['additional_dependencies'])
        if returncode != 0:
    ...
    for idx in failed_count:
        print(f"idx {idx} \t Library: {df.loc[idx, 'library']}, Version: {df.loc[idx, 'version']}, Additional Dependencies: {df.loc[idx, 'additional_dependencies']}")
