...
class TestGammaLn(unittest.TestCase):
    ...
    def test_basic_positive_values(self):
        ...
        self.assertTrue(torch.isclose(result[4], torch.tensor(np.log(2.0), dtype=torch.float32)))
    ...
    def test_integer_values(self):
        ...
        self.assertTrue(torch.isclose(result[0], torch.tensor(0.0)))  # gamma_ln(1) = ln(0!) = ln(1) = 0
        self.assertTrue(torch.isclose(result[1], torch.tensor(0.0)))  # gamma_ln(2) = ln(1!) = ln(1) = 0
    ...
    def test_compare_with_scipy_implementation(self):
        ...
        torch.testing.assert_close(gamma_ln_x_plus_1, expected_relation, rtol=1e-10, atol=1e-10)
