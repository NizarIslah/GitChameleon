...
class TestComputeMfccToMel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y = np.sin(2 * np.pi * 440 * np.linspace(0, self.duration, int(self.sr * self.duration)))
        
        # Compute MFCCs from the audio signal
        self.n_mfcc = 13
        self.n_mels = 128
        self.mfcc = librosa.feature.mfcc(y=self.y, sr=self.sr, n_mfcc=self.n_mfcc, n_mels=self.n_mels)
    ...
    def test_compute_mfcc_to_mel_custom_params(self):
        ...
        mel_power = compute_mfcc_to_mel(self.mfcc, n_mels=n_mels, dct_type=dct_type, norm=norm, ref=ref)
