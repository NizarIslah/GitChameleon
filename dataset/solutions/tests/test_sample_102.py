...
class TestSaveExisting(unittest.TestCase):
    ...
    def test_save_existing_calls_formset_method(self):
        ...
        result = save_existing(formset=self.mock_formset, form=self.mock_form, instance=self.instance)
        
        # Assert that the formset's save_existing method was called with the correct parameters
        self.mock_formset.save_existing.assert_called_once_with(form=self.mock_form, obj=self.instance)
    ...
    def test_save_existing_with_different_instance(self):
        ...
        save_existing(formset=self.mock_formset, form=self.mock_form, instance=different_instance)
        
        # Assert that the formset's save_existing method was called with the correct parameters
        self.mock_formset.save_existing.assert_called_once_with(form=self.mock_form, obj=different_instance)
