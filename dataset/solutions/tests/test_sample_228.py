...
def test_pytest_ignore_collect_signature():
    ...
    # Check that the parameter is annotated with pathlib.Path
...
def test_pytest_hookimpl_decorator():
    ...
    # This is a bit tricky to test directly, but we can check if the function
    # has the attributes that pytest adds to hook implementations
    assert hasattr(sample_228.pytest_ignore_collect, 'pytestPrefixed')