...
from sympy.physics.mechanics import (
    RigidBody, PinJoint, PrismaticJoint, Point, ReferenceFrame, 
    inertia, dynamicsymbols
)
...
class TestCustomMotion(unittest.TestCase):
    def setUp(self):
        ...
        self.body1 = RigidBody('body1', masscenter=self.P1, frame=self.A, 
                              mass=self.m1, inertia=(self.I1, self.P1))
        self.body2 = RigidBody('body2', masscenter=self.P2, frame=self.B, 
                              mass=self.m2, inertia=(self.I2, self.P2))
        ...
        # Create joints
        self.slider = PrismaticJoint('slider', self.wall, self.body1, 
                                    self.q[0], self.u[0], self.N.x)
        self.pin = PinJoint('pin', self.body1, self.body2, 
                           self.q[1], self.u[1], self.N.z)
    ...
    def test_custom_motion_with_different_bodies(self):
        ...
        new_body = RigidBody('new_body', masscenter=self.P1, frame=self.A, 
                            mass=self.m1, inertia=(self.I1, self.P1))
        
        # Create new joints
        new_slider = PrismaticJoint('new_slider', self.wall, new_body, 
                                   self.q[0], self.u[0], self.N.x)
        new_pin = PinJoint('new_pin', new_body, self.body2, 
                          self.q[1], self.u[1], self.N.z)
