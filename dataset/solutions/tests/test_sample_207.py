...
class TestCustomLegendre(unittest.TestCase):
    ...
    def test_legendre_properties(self):
        ...
        for p in primes:
            a = 25  # 25 mod 7 = 4
            self.assertEqual(custom_legendre(a, p), custom_legendre(a % p, p), 
                            f"Failed property (a|p) = (a mod p|p) for a={a}, p={p}")
        
        # Property 2: (1|p) = 1 for any odd prime p
        for p in primes:
            self.assertEqual(custom_legendre(1, p), 1, f"Failed property (1|p) = 1 for p={p}")
        
        # Property 3: (ab|p) = (a|p)(b|p) for any odd prime p
        for p in primes:
            a, b = 2, 3
            self.assertEqual(custom_legendre(a * b, p), 
