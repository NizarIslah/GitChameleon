...
class TestSample104(unittest.TestCase):
    ...
    def test_rendered_output_matches_target(self):
        ...
        helptext_pos = normalized_html.find('<div class="helptext" id="id_name_helptext">')
        input_pos = normalized_html.find('<input type="text" name="name"')
        
        self.assertGreater(label_pos, 0, "Label tag not found")
        self.assertGreater(helptext_pos, label_pos, "Help text div not found or in wrong order")
        self.assertGreater(input_pos, helptext_pos, "Input field not found or in wrong order")
