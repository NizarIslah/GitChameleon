...
from dataset.solutions.sample_132 import apply_percentile_filter
...
class TestPercentileFilter(unittest.TestCase):
    ...
    def test_apply_percentile_filter_different_percentiles(self):
        ...
        for percentile in [0, 25, 50, 75, 100]:
            ...
            expected = np.zeros_like(test_array)
            for i in range(test_array.shape[0]):
                expected[i] = percentile_filter(test_array[i], percentile=percentile, size=3)
            
            # Check if the result matches the expected output
            np.testing.assert_array_equal(
                result, expected, 
                err_msg=f"Failed with percentile={percentile}"
            )
    ...
    def test_apply_percentile_filter_different_sizes(self):
        ...
        for size in [1, 3, 5]:
            ...
            np.testing.assert_array_equal(
                result, expected, 
                err_msg=f"Failed with size={size}"
            )
