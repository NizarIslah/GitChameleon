...
class TestSample222(unittest.TestCase):
    ...
    def test_solution_adds_method(self):
        ...
        self.assertFalse(hasattr(ConnectionLogger, 'client_disconnected') and 
                         callable(getattr(ConnectionLogger, 'client_disconnected')))
        
        # Call solution
        solution()
        
        # Verify method exists after calling solution
        self.assertTrue(hasattr(ConnectionLogger, 'client_disconnected') and 
                        callable(getattr(ConnectionLogger, 'client_disconnected')))
