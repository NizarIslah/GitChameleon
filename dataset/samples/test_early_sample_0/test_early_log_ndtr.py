...
class TestLogNdtr:
    ...
    def test_log_ndtr_positive_values(self):
        ...
        expected_output = torch.from_numpy(
            np.log(np.array([0.691462, 0.841345, 0.977250]))
        )
    ...
    def test_log_ndtr_negative_values(self):
        ...
        input_tensor = torch.tensor([-0.5, -1.0, -2.0], dtype=torch.float32)
        expected_output = torch.from_numpy(
            np.log(np.array([0.308538, 0.158655, 0.022750]))
        )
        output = log_ndtr(input_tensor)
        assert torch.allclose(
            output, expected_output, atol=1e-5
    ...
    def test_log_ndtr_non_finite_values(self):
        ...
        assert (
            output[1] == 0.0
        ), f"Expected 0.0 for positive infinity, but got {output[1]}"
        assert output[2] == float(
            "-inf"
        ), f"Expected -inf for negative infinity, but got {output[2]}"
