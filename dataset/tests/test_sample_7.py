...
class TestErf(unittest.TestCase):
    ...
    def test_basic_values(self):
        ...
        # erf(0) should be 0
        self.assertTrue(torch.isclose(result[2], torch.tensor(0.0, dtype=torch.float32)))
    ...
    def test_large_values(self):
        ...
        self.assertTrue(torch.isclose(result[0], torch.tensor(1.0, dtype=torch.float32), atol=1e-4))
        self.assertTrue(torch.isclose(result[2], torch.tensor(1.0, dtype=torch.float32), atol=1e-9))
        
        # erf(x) approaches -1 as x approaches negative infinity
        self.assertTrue(torch.isclose(result[1], torch.tensor(-1.0, dtype=torch.float32), atol=1e-4))
        self.assertTrue(torch.isclose(result[3], torch.tensor(-1.0, dtype=torch.float32), atol=1e-9))
    ...
    def test_erf_properties(self):
        ...
        # erf(0) = 0
        self.assertTrue(torch.isclose(erf(torch.tensor([0.0], dtype=torch.float64))[0], 
