...
class TestBesselI1(unittest.TestCase):
    ...
    def test_basic_positive_values(self):
        ...
        # I₁(0) = 0
        self.assertTrue(torch.isclose(result[0], torch.tensor(0.0, dtype=torch.float32)))
    ...
    def test_relationship_with_i0(self):
        ...
        torch.testing.assert_close(i1_derivative, expected_derivative, rtol=1e-3, atol=1e-3)
    ...
    def test_bessel_i1_properties(self):
        ...
        # I₁(0) = 0
        self.assertTrue(torch.isclose(bessel_i1(torch.tensor([0.0], dtype=torch.float64))[0], 
        ...
        # For x > 0, I₁(x) < I₀(x)
        from scipy.special import i0 as bessel_i0
        x_positive = torch.linspace(0.1, 5.0, 100, dtype=torch.float64)
        i1_x_positive = bessel_i1(x_positive)
        i0_x_positive = bessel_i0(x_positive)
        
        self.assertTrue(torch.all(i1_x_positive < i0_x_positive))
