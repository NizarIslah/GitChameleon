...
def pass_cols(table):
    return [
        ...
        if col_pass_pattern in col and output_pattern in col and "regen" not in col
    ]
...
def out_cols(table):
    return [
        col
        for col in table.columns
        if output_pattern in col and "compile" not in col and "pass" not in col
    ]
...
def validate_tables(table1, table2, model_name):
    ...
    assert np.all(
        pass_indices1 == pass_indices2
    ), f"Pass indices are not the same, {sum(pass_indices1)}, {sum(pass_indices2)}"

    print(
        table1[f"{model_name}_pass_at_10"].mean(),
        table2[f"{model_name}_pass_at_10"].mean(),
    )
...
def parse_option():
    ...
    parser.add_argument("--output-path", type=str, default="results/results.csv")
        ...
        py_files = [
            make_py_file(
                ...
                py_file=os.path.join(
                    tmp_path, f"temp_{idx}_{k}_{options.test_prompt}.py"
                ),
            ...
            with open(options.cot_output_path, "r") as f:
                for line_idx, line in enumerate(f):
                    ...
                    for k in range(options.n_generate):
                        outputs[f"{model_name}_output_{k}"].append(
                            sanitize(resp["solution"])
                        )
